...
const Tvshows = () => {
  ...
  const fetchTvShows = async () => {
    try {
      console.log(`Fetching: /tv/${category}?page=${page}`); // Debugging log
      ...
      console.error("Error fetching TV shows: ", error.response || error);
    }
  };

  // Reset state when category changes
  useEffect(() => {
    setTvShows([]); // Clear previous results
    setPage(1); // Reset page
    setHasMore(true); // Reset infinite scroll
    fetchTvShows(); // Fetch new category data
    ...
    <div className="min-h-screen w-screen p-6 overflow-hidden overflow-y-auto">
      ...
      <div className="w-full flex items-center space-x-4">
        ...
        <Dropdown
          ...
          func={(e) => {
            setCategory(e.target.value); // Change category
          }}
        />
      </div>
